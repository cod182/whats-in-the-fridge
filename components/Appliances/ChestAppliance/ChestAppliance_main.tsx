          ...
          {appliance.fridgeCompartment && appliance.fridgeCompartment.map(({ shelves, drawers }: CompartmentProps, index: number) => (
            <div key={index} className="w-full h-full">
              ...
              {drawers != undefined &&
                <div className={`flex flex-row items-center gap-2 h-full w-full`}>
                  {drawers.map((drawerNum, index) => (
                    <ChestStorageArea key={index} handleSelection={handleSelect} compartment='fridge' type='drawer' level={drawerNum} handleModalState={handleModalState} items={getItemsInThisLocation(drawerNum, itemsInLocation.fridge, 'drawer', 'fridge', 128)} />
                  ))}
                </div>
              }
            </div>
