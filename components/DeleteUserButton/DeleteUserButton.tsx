...
const DeleteUserButton = ({ user, apiRoute }: Props) => {
  // States
  ...
  return (
    <div className='w-[200px] h-full my-2 flex flex-row items-center' >
      ...
      <div className={`flex flex-row overflow-hidden items-center justify-center gap-2  h-full group ${confirmButton ? 'w-full' : 'w-0'} transition-all duration-200 ease`}>
        <button className="flex flex-row items-center justify-around w-full px-2 border-b-4 border-black rounded-lg hover:border-b-0 hover:translate-y-1 hover:mb-1 bg-red-500/50 hover:bg-red-500/80 hover:font-semibold" onClick={() => handleCheck()}>
          <TiWarning />
          <span>
            Delete
          </span>
        ...
        <button className="w-full px-2 border-b-4 border-black rounded-lg hover:border-b-0 hover:translate-y-1 hover:mb-1 bg-blue-500/50 hover:bg-blue-500/80 hover:font-semibold" onClick={() => { setCheckDelete(true); setConfirmButton(false); }}>Cancel</button>
      </div>
